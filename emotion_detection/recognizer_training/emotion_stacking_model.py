...
try:
    ...
    print("XGBoost not available, falling back to Ridge regression only")
...
def train_lgb(X_text, X_emo, y):
    ...
    for i in range(4):
            gbm = xgb.XGBRegressor(n_estimators=100, learning_rate=0.1, random_state=42)
        gbm.fit(X, y[:, i])
        models.append(gbm)
    else:
        # Fallback to additional Ridge models if XGBoost is not available
        for i in range(4):
